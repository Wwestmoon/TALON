...
def check_denotation(target, pred):
...
def main(file_path):
    ...
    if os.path.isdir(file_path):
        ...
        for filename in os.listdir(file_path):
            if filename.endswith(".json"):
                ...
                with open(os.path.join(file_path, filename), 'r', encoding='utf-8') as f:
                    data = json.load(f)
                # print(data)
                pred_answer.append(data['answer'])
                gold_answer.append(data['label'])
            if count_len>230:
                break
        ...
        for i in range(len(pred_answer_val)):
            if check_denotation( pred_answer_val[i], gold_answer_val[i]):
                count += 1
            # else:
            #     print(f"Predicted: {pred_answer_val[i]}")
            #     print(f"Gold: {gold_answer_val[i]}")
            #     print("Not match")
        accuracy = count / len(pred_answer_val) * 100
        ...
        alltoken=0
        for i in data:
                # pred_answer.append(i['current_pred'][0])
                gold_answer.append(i['gold'])
                # token=i['token']["total_tokens"]
                # alltoken+=token
        print(len(pred_answer)) 
        pred_answer_val = to_value_list(pred_answer)
        gold_answer_val = to_value_list(gold_answer)
        # correct = (pred_answer_val, gold_answer_val)
        print(count_50)
        for i in range(len(pred_answer_val)):
            if check_denotation( pred_answer_val[i], gold_answer_val[i]):
                count += 1
        ...
        accuracy = count / len(pred_answer_val) * 100
        print(f"Accuracy: {accuracy:.2f}%")
        print(f"Average token count: {alltoken/len(pred_answer_val):.2f}")
